    ...
    <Suspense
      fallback={
        <div className="max-w-7xl mx-auto">
          <SmallLoader subtitle="Loading form..." />
        </div>
      }
    >
      <CreateTestForm />
...
function CreateTestForm() {
  ...
  const [selectedPaper, setSelectedPaper] = useState<PaperWithModules | null>(
    null,
  );
  ...
  // Data state
  const [papers, setPapers] = useState<PaperWithModules[]>([]);
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    const data = await fetchActivePapers(currentCenter.center_id);
    setPapers(data);
    setLoading(false);
  };
  ...
  const loadTestData = async () => {
    ...
    if (testData) {
      ...
      // Parse scheduled_at into local date and time
      const d = new Date(testData.scheduled_at);
  ...
  const handlePaperSelect = (paper: PaperWithModules) => {
